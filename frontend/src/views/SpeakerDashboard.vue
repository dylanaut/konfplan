    ...
    <section class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input v-model="speaker.organization" type="text" class="input-field" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700">Rolle / Position</label>
          <input v-model="speaker.jobRole" type="text" class="input-field" />
        </div>
        <div class="md:col-span-2">
          <label class="block text-sm font-medium text-gray-700">E-Mail Adresse</label>
          <input v-model="speaker.email" type="email" class="input-field" />
        </div>
      </div>
    </section>
...
onMounted(async () => {
  const [userRes, talkRes, slotRes] = await Promise.all([
    api.get('/api/speaker/profile'),
    api.get('/api/speaker/my-talk'),
    api.get('/api/slots') // Globaler Endpunkt für alle Event-Slots
  ]);
  speaker.value = userRes.data;
...
const saveAll = async () => {
  try {
    await Promise.all([
      api.put('/api/speaker/profile', speaker.value),
      api.put('/api/speaker/my-talk', { ...talk.value, availabilities: availabilities.value })
    ]);
    alert("Gespeichert!");
  } catch (e) {
    alert("Fehler!");
