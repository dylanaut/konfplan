<template>
  <div class="container mt-5">
      ...
      <table class="table table-striped table-bordered">
        <thead class="table-dark">
          <tr class="print-header-row">
            <th colspan="3" class="p-0 border-0 bg-white text-body">
    ...
    <ReportFooter :veranstaltung-name="reportData.veranstaltung.name"
                  :report-titel="`Laufzettel für ${reportData.referent.firstName} ${reportData.referent.lastName}`" />
  </div>
</template>
...
onMounted(async () => {
  ...
  try {
    ...
    const url = `/api/reports/${veranstaltungId}/referent/${referentId}/laufzettel-data` + (ergebnisId ? `?ergebnisId=${ergebnisId}` : '');
