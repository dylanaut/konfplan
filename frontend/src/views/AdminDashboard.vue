    ...
    <div class="border-b border-gray-200">
      <nav class="-mb-px flex space-x-6 overflow-x-auto">
        <button v-for="tab in visibleTabs" :key="tab"
                @click="activeTab = tab"
                :class="[activeTab === tab ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm capitalize']">
          {{
            ...
            <tr v-if="selectedVid === v.id" class="bg-gray-50/50">
              <td colspan="3" class="px-4 py-4 space-y-6">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                      <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 text-[8px] uppercase font-bold text-gray-500">
                          <tr>
                            <th class="px-3 py-1.5 text-left">Titel</th>
                            <th class="px-3 py-1.5 text-left">Referent</th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 text-[10px]">
                          <tr v-for="talk in vortraege" :key="talk.id" class="hover:bg-indigo-50/30 transition">
                            ...
                            <td class="px-3 py-1.5 text-gray-600" :title="talk.referent?.email">
                              {{ talk.referent?.lastName }}, {{ talk.referent?.firstName }}
                            </td>
                          </tr>
                          <tr v-if="vortraege.length === 0">
                            <td colspan="2" class="px-3 py-4 text-center text-gray-400 italic">Keine Vorträge angelegt.</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                  ...
                  <!-- Untertabelle: Teilnehmer & Prioritäten -->
                  <div class="space-y-2">
                    <h4 class="text-[10px] font-black text-indigo-700 uppercase tracking-widest border-b border-indigo-100 pb-1 flex items-center gap-2">
                      ...
                      <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 text-[8px] uppercase font-bold text-gray-500">
                          <tr>
                            ...
                            <th class="px-3 py-1.5 text-left">Prios (Vortrag ID:Rang)</th>
                          </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 text-[10px]">
                          <tr v-for="part in participants" :key="part.id" class="hover:bg-indigo-50/30 transition">
                            <td class="px-3 py-1.5 font-semibold text-gray-800" :title="part.email">
                              {{ part.lastName }}, {{ part.firstName }}
                            </td>
                            <td class="px-3 py-1.5 text-gray-500 font-mono text-[9px]">
                              {{ part.prioritaeten?.map(p => `${p.vortragId}:${p.prioWert}`).join(', ') || '-' }}
                            </td>
                          </tr>
                          <tr v-if="participants.length === 0">
                            <td colspan="2" class="px-3 py-4 text-center text-gray-400 italic">Keine Teilnehmer angemeldet.</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                    ...
                    <tr v-for="r in sortRaeume(g.raeume)" :key="r.id" class="hover:bg-gray-50 transition">
                      ...
                      <td class="px-3 py-1 text-right space-x-2">
                        <button @click="openRaumEditor(r, g.id)" class="text-indigo-600" title="Bearbeiten">
                          <PencilIcon class="w-3 h-3 inline"/>
                        </button>
