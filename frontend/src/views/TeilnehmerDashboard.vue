        ...
        <div v-for="event in events" :key="'schedule-' + event.id" class="border border-gray-200 rounded-lg p-4">
          <div class="flex justify-between items-center">
            ...
            <div v-if="event.planErstellt" class="flex gap-2">
              <button @click="downloadKalender(event.id)" class="btn-secondary">
            ...
            <table v-else class="w-full text-sm">
              <thead>
                <tr class="text-left text-xs text-gray-500 border-b border-gray-200">
                  ...
                  <th class="py-1 pr-4 font-medium">Raum</th>
                  <th class="py-1 font-medium">Referent</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(eintrag, idx) in laufzettelByEvent[event.id]" :key="idx" class="border-b border-gray-100 last:border-0">
                  ...
                  <td class="py-1.5 pr-4 text-gray-600">{{ eintrag.raumName }}</td>
                  <td class="py-1.5 text-gray-600">{{ eintrag.referentName }}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="flex flex-col">
            ...
            <div v-if="activeEventId === event.id" class="p-4 border-t border-gray-200 bg-white animate-fade-in">
              <div v-if="vortraege.length > 0">
                ...
                <h4 class="font-bold text-sm mb-2">Wähle hier Deine Prioritäten</h4>
...
const fetchLaufzettelForEvent = async (vid) => {
   ...
   try {
      const res = await api.get(`/api/reports/${vid}/teilnehmer/${profile.value.id}/laufzettel-data`);
