    ...
    <div class="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
      <div class="flex items-center gap-4">
        ...
        <div v-if="selectedParticipantIds.length > 0" class="flex items-center gap-2 animate-fade-in bg-indigo-50 px-3 py-1 rounded-lg border border-indigo-100">
          ...
          <button @click="emit('batchDeactivateParticipants', selectedParticipantIds)" class="text-xs text-orange-600 hover:underline font-bold">Deaktivieren</button>
          <button @click="emit('batchDeleteParticipants', selectedParticipantIds)" class="text-xs text-red-600 hover:underline font-bold">Löschen</button>
          <button @click="emit('batchEmailParticipants', selectedParticipantIds)" class="text-xs text-indigo-600 hover:underline font-bold flex items-center gap-1">
      ...
      <div class="flex gap-2">
        ...
        <button @click="emit('openUserModal', {role: 'TEILNEHMER'})" class="btn-primary text-xs py-1 px-3">+ Neu</button>
      </div>
    </div>
