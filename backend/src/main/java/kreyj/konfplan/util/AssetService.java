...
public class AssetService {
    ...
    public AssetService() {
        // Nur im Produktionsmodus versuchen, das Manifest zu laden
        if (LaunchMode.current().isProduction()) {
            loadManifest();
            if (manifest == null) {
                LOG.warn("manifest.json konnte im Produktionsmodus nicht geladen werden. Asset-Pfade sind möglicherweise falsch.");
            }
        } else {
            // Im Dev-Modus ist das Manifest nicht vorhanden, das ist normal.
            manifest = null;
            LOG.info("Anwendung im DEV-Modus. Asset-Manifest wird nicht geladen. Das ist erwartet.");
        }
    }
