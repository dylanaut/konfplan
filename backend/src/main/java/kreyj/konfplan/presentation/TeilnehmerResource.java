...
public class TeilnehmerResource {
    ...
    public Response getTeilnehmer(@PathParam("id") Long id) {
        ...
        if (null == nutzer) return Response.status(Response.Status.NOT_FOUND).build();
    ...
    // ========== Teilnehmer-spezifische Endpunkte ==========
    ...
    public Response getTeilnehmerProfile() {
        ...
        if (null == teilnehmer) throw new WebApplicationException("Teilnehmer not found", Response.Status.NOT_FOUND);
    ...
    public Response updateTeilnehmerProfile(@RequestBody(description = "Die aktualisierten Profildaten") NutzerDto teilnehmerDto) {
        if (teilnehmerDto == null) return Response.status(Response.Status.BAD_REQUEST).build();
    ...
    public Response getMyLaufzettel(@PathParam("vid") Long vid) {
        Teilnehmer teilnehmer = teilnehmerService.findByEmail(JwtHelper.getUserPrincipalName(jwt));
        if (teilnehmer == null) return Response.status(Response.Status.NOT_FOUND).entity("Teilnehmer nicht gefunden.").build();
    ...
    public Response getPlan(@QueryParam("vid") Long vid) {
        ...
        List<ZuweisungDto> planFuerTeilnehmer = planService.getPlanFuerTeilnehmer(JwtHelper.getUserPrincipalName(jwt), veranstaltung);
    ...
    public NutzerVerfuegbarkeitDto getVerfuegbarkeiten(@PathParam("vid") Long vid) {
        ...
        if (null == nv) throw new WebApplicationException("Keine Verfügbarkeit für diesen Nutzer und diese Veranstaltung gefunden.", Response.Status.NOT_FOUND);
    ...
    public Response updateVerfuegbarkeit(@PathParam("vid") Long vid, @RequestBody(description = "Die Verfügbarkeitsdaten") NutzerVerfuegbarkeitDto dto) {
        Nutzer nutzer = Nutzer.findByEmail(JwtHelper.getUserPrincipalName(jwt));
        if (!(nutzer instanceof Teilnehmer) || !nutzer.getId().equals(dto.nutzerId)) return Response.status(FORBIDDEN).build();
        Veranstaltung veranstaltung = Veranstaltung.findById(vid);
        if (veranstaltung == null) return Response.status(Response.Status.NOT_FOUND).build();
        ...
        if (v == null) return Response.status(Response.Status.NOT_FOUND).entity("Verfügbarkeitseintrag nicht gefunden.").build();
