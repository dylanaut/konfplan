...
- **Password policy** is two-tiered (see #741): Keycloak's realm-level `passwordPolicy` (`konfplan-realm.json`/`ProdKeycloakRealmSyncService`) is now just a low floor (`length(6)`) that applies to *every* newly-set password, including admin-set ones — Keycloak enforces its realm policy for every credential-setting call, not just self-service, so the floor can never be relaxed per role there. The actual, meaningful password strength for admin-set passwords (single reset, ZIP bulk-generation) is enforced entirely app-side by `PasswortrichtlinieService`, configurable per Veranstaltung and Nutzer role (falls back to `Passwortrichtlinie.STANDARD` — 8 chars, all four character classes — if nothing is configured). Keycloak's own self-service "forgot password" flow is bound only by the low realm floor, not by any per-role `Passwortrichtlinie`.
